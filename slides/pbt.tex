...
\newcommand{\checkmak}{\text{\ding{52}}}
\newcommand{\crossmak}{\text{\ding{54}}}
...
\item Write \emph{properties} instead of specific tests
  \begin{itemize}
  \item functions that take some arguments and assert something
  \item should be universal, i.e.\@ hold for \emph{all} values
  \item should define the expected behaviour for \emph{all} cases
\end{itemize}
\item Specify \emph{generators} for the inputs
\item The testing framework executes the property with
...
\begin{itemize}
\item Haskell QuickCheck (2000): first PBT library 
...
\begin{frame}
  ...
  \begin{itemize}
  \item Hypothesis attempts to \alert{simplify counter-examples}
    before presenting; e.g.:
  \begin{itemize}
  \item removing elements from any lists
  \item shrinking elements inside any lists
  \end{itemize}
\item This is useful to remove ``noise'' from
  randomly generated data
\item Hypothesis keeps shrinking recursively while example still fails
  the property
\end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  A wrong property on lists: 
\begin{verbatim}
@given(st.lists(st.integers())
def test_reverse_wrong(x):
    assert list(reversed(x)) == x
\end{verbatim}
\medskip
\pause
  
\begin{semiverbatim}
pytest basic.py -k test_reverse_wrong

\alert{E       Falsifying example: test_reverse_wrong(}
\alert{E           x=[0, 1],}
\alert{E       )}
\end{semiverbatim}
...
\begin{itemize}
\item The counter-example reported is \alert{minimal}
\item \texttt{[]}, \texttt{[0]}, \texttt{[1]} and
  \texttt{[0,0]}
  are \emph{not} counter-examples
...
\begin{frame}[allowframebreaks]
  \frametitle{State machine testing}

  \begin{itemize}
  \item Start from a well-defined initial state
  \item Generate random sequences of commands
  \item Executed  both in the \alert{implementation} and 
    some \alert{model}
  \item Compare the results of implementation against the model
    after each step
\end{itemize}

\[\xymatrix{
    \text{implementation} &
    s_0 \ar[r]^{r_1} & s_1\ar[r]^{r_2}\ar@{--}[d]^{?} & s_2\ar[r]^{r_3}\ar@{--}[d]^{?} & s_3\ar[r]^{r_4}\ar@{--}[d]^{?} & \cdots \\
    \text{model} & 
    m_0\ar[r]^{r_1} & m_1\ar[r]^{r_2} & m_2\ar[r]^{r_3} & m_3\ar[r]^{r_4} & \cdots
  }
\]
...
\begin{itemize}
\item Tests fail if there is a mismatch between model and implementation
\item The counter example is the \alert{command sequence} leading to the failure
\item Shriking is applied to find a minimal command sequence 
\end{itemize}
 
\[ \xymatrix{
    \text{implementation} &
    s_0 \ar[r]^{r_1} & s_1\ar[r]^{r_2}\ar@{--}[d]^{\checkmak} & s_2\ar[r]^{r_3}\ar@{--}[d]^{\checkmak} & s_3\ar@{--}[d]^{\crossmak} &  \\
    \text{model} & 
    m_0\ar[r]^{r_1} & m_1\ar[r]^{r_2} & m_2\ar[r]^{r_3} & m_3 & 
  }
\]

\begin{center}
  Sequence $[r1,r2,r3]$ leads to a test failure.
  \end{center}
...
\begin{frame}
  \frametitle{State machine testing example}

  \begin{itemize}
  \item A database for associating \emph{users} and \emph{emails}
  \item Implemented as stateful class and SQLite 
  \item Users are represented by an integer ids
  \item Each user can have more than one email
  \item Operations:
    \begin{description}
    \item[add(userid,email)]  add an email to a user
    \item[get(userid)] get all emails from a user
    \item[remove(userid,email)] remove and email from a user
    \item[delete(userid)] remove all emails from a user
    \end{description}
  \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{Testing the database}

  \begin{itemize}
  \item The model is a list of pairs (user,email)
  \item The state machine initializes starts with and empty DB
    and empty model
  \item Each transition is defined as a method labeled as \alert{rule}
    \begin{itemize}
    \item corresponds to one of the UserDB operations
    \item performs the operation in both implementation and model
    \item checks that the implementation and model match (if applicable)
    \end{itemize}
  \item Hypothesis will generate random sequences of rules
  \item A counter-example will be a \alert{sequence of rules}
